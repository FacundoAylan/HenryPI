...
export const getCountries = () =>{
    return async (dispatch) =>{
        try{
            const json = await axios.get('http://localhost:3006/countries');
            dispatch({
                type: GET_COUNTRIES,
                payload: json.data
            })
...
export const getActivities = () => {
    return async function (dispatch) {
        try {
            let json = await axios.get('http://localhost:3006/activities');
            return dispatch({
                type: GET_ACTIVITIES,
                payload: json.data
            })
...
export const addActivity = (payload) =>{
    return async (dispatch) => {
        try{
            return await axios.post('http://localhost:3006/activities',payload)
        }catch(err){
            console.log(err);
        }
    }
...
export const detail= (id) => {
    return async (dispatch) => {
        try {
            var json = await axios.get(`http://localhost:3006/countries/${id}`)
            return dispatch({
                type: DETAIL,
                payload: json.data
            });
